...
(define (parse-rule line)
  (let* ((parts (regexp-split " bags contain " line))
         (outer-bag (first parts))
         (inner-bags (parse-inner-bags (second parts))))
    (values outer-bag inner-bags)))

(define (contains? rules cache bag want)
  (or (hash-table-ref/default cache bag #f)
      (let ((result (or (string=? bag want)
                        (any (lambda (p)
...
(define (process-rules-1 lines)
  (let ((rules (make-hash-table string-comparator))
    ...
    (for-each (lambda (line)
                (let-values (((outer-bag inner-bags) (parse-rule line)))
                  (hash-table-set! rules outer-bag inner-bags)))
              lines)
    (count (lambda (bag)
